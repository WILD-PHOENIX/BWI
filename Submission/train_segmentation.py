...
class MaskDataset(Dataset):
    def __init__(self, data_dir, transform=None, mask_transform=None, augment=False, target_size=(266, 476)):
...
def main():
    # Configuration
    # Note: RTX 5050 (sm_120) not yet supported by PyTorch, using CPU
    # Change to 'cuda' once PyTorch adds support for Blackwell GPUs
    ...
    trainset = MaskDataset(data_dir=data_dir, augment=True, target_size=(h, w))
    train_loader = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=0)

    valset = MaskDataset(data_dir=val_dir, augment=False, target_size=(h, w))
